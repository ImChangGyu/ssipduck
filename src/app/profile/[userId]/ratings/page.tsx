...
export async function generateMetadata({ params }: PageProps) {
  ...
  const { data: profile } = await supabase
    ...
    .eq('id', userId)
    .maybeSingle();
  return { title: profile ? `${profile.nickname}님의 평가 | 씹덕` : '평가 목록 | 씹덕' };
}

async function RatingList({ userId }: { userId: string }) {
  const supabase = await createClient();

  const [{ data: profileData }, { data: ratings }] = await Promise.all([
  ...
  if (ids.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-on-surface-variant gap-2">
        <p className="text-body-lg">평가한 작품이 없습니다.</p>
      </div>
    );
  }
...
export default async function UserRatingsPage({ params }: PageProps) {
  const { userId } = await params;
  const supabase = await createClient();
  const { data: profile } = await supabase
    .from('profiles')
